...
static inline float bilinearInterpolation(float dy, float dz,
...
// There will be a small peak at min and max
// (since normal distributions are not really supposed to be limited to a range)
struct NormalDistributionGenerator {
    NormalDistributionGenerator(float min = 0.0f,
                                float max = glm::half_pi<float>())
...
static inline glm::vec3 sampleHemisphereUniform(const glm::vec3& n)
{
    ...
    assert(glm::length(glm::cross(nonParallellVector,
...
static inline float schlicksApprox(
